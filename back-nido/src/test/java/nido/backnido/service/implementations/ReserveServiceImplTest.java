...
public class ReserveServiceImplTest {
    ...
    @Test
    public void getReserveByIdTest_Ok(){
        Product product = new Product();
        User user = new User();
        Reserve reserve = new Reserve(1L,LocalDate.of(2021,12,10),
                LocalDate.of(2021,12,12),
        ...
        when(reserveRepository.findById(anyLong())).thenReturn(Optional.of(reserveResponse));
        reserveService.getById(1L);

        verify(reserveRepository).findById(reserve.getReservationId());
        assertEquals(reserve.getDateIn(), reserveResponse.getDateIn());
        assertEquals(1L, reserveResponse.getReservationId());
